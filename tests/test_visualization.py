...
class VisualizationTests(unittest.TestCase):
    def test_visualize_creates_plotly_dashboard(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn('content="explainable-ko-v3-no-report-drawer"', html)
            ...
            self.assertNotIn("developer-drawer", html)
            self.assertNotIn("developer-toggle", html)
            self.assertNotIn("개발자 정보", html)
            self.assertNotIn("toggledeveloperdrawer", html)
